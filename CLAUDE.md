...
3. **`handle_follow_up`** (`tasks/follow_up.py`, per-profile) — sends follow-up message to one CONNECTED profile → COMPLETED. On rate limit, reschedules with 1h delay.
...
### CRM Data Model
- **Campaign** (`linkedin.models.Campaign`) — 1:1 with `common.Department`. Stores `product_docs`, `campaign_objective`, `followup_template` (Jinja2 prompt content), `booking_link`, `is_freemium` (bool), `action_fraction` (float, target fraction of total connections for freemium campaigns; used to scale reschedule delay based on elapsed execution time).
...
- **`setup/freemium.py`** — `import_freemium_campaign(kit_config)` creates/updates a freemium `Campaign` with `is_freemium=True`. (Moved from `ml/hub.py`; `setup/partner.py` remains as a backwards-compat shim.)
...
- **`setup/self_profile.py`** — Self-profile sentinel logic. (Moved from `self_profile.py`.)
...
- **`renderer.py`** — Two-stage template rendering. `invoke_llm(prompt)` creates `ChatOpenAI` with `temperature=0.7` and `AI_MODEL`. `render_template(session, template_content, profile)` first renders Jinja2 template (with profile context + `product_description` from `session.campaign.product_docs`), then passes result through `invoke_llm()`, then appends `booking_link` from campaign (after LLM call, not part of prompt). (Moved from `templates/renderer.py`; `templates/renderer.py` remains as a backwards-compat shim.)
...
- **`actions/message.py`** — `send_follow_up_message(session, profile) → str | None`: checks connection status first, renders template via `render_template()` from `renderer.py`, sends via popup (`_send_msg_pop_up`) or direct messaging thread (`_send_message`). Returns message text or None. Has CLI `__main__` block.
