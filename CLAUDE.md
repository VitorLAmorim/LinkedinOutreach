...
The daemon (`daemon.py`) runs continuously with multi-campaign support using a **persistent task queue** backed by the `Task` Django model. Tasks are ordered by `scheduled_at` timestamp; the worker loop pops the oldest due task, executes it, and each task handler self-schedules follow-on tasks. On startup, `heal_tasks()` reconciles the queue with CRM state (recovers stale running tasks, seeds missing tasks). Daily/weekly rate limiters independently cap totals (shared across campaigns). Partner campaigns have a dedicated `connect_partner` task type with scheduling-based rate control via `action_fraction`.

Four task types (all plain handler functions in `linkedin/tasks/`, signature: `handle_*(task, session, qualifiers, partner_qualifier, kit_model)`):

1. **`handle_connect`** (`tasks/connect.py`, per-campaign) — Regular campaigns only. Pull-based architecture. Calls `get_candidate()` from `pools.py` which uses composable generators: `ready_source` → `qualify_source` → `search_source`. Each level tries its own pool, then pulls ONE item from upstream if empty. If a candidate is found, connects → PENDING, enqueues `check_pending` task for that profile. Self-reschedules with short delay (~10s) to burn daily connection quota. On rate limit or no candidates, reschedules with longer delay.
2. **`handle_connect_partner`** (`tasks/connect_partner.py`, per-campaign) — Partner campaigns only. Same connection logic as regular connect but uses `partner_qualifier` + `kit_model` for candidate selection, `threshold=0` (all QUALIFIED promoted unconditionally), and calls `seed_partner_deals()` each iteration. Self-reschedules with delay = `connect_delay_seconds / action_fraction` (e.g. fraction=0.2 → 50s delay, yielding 1 partner connect per 5 regular connects).
3. **`handle_check_pending`** (`tasks/check_pending.py`, per-profile) — checks one PENDING profile for acceptance → CONNECTED. Uses exponential backoff with multiplicative jitter: `delay = backoff * uniform(1.0, 1.0 + jitter_factor)`. Doubles `backoff_hours` each time a profile is still pending. On acceptance, enqueues `follow_up` task.
4. **`handle_follow_up`** (`tasks/follow_up.py`, per-profile) — sends follow-up message to one CONNECTED profile → COMPLETED. On rate limit, reschedules with 1h delay.
...
3. **READY_TO_CONNECT gate** — After LLM qualifies a profile to QUALIFIED, it must also pass a GP confidence gate before becoming connectable. `promote_to_ready()` computes P(f > 0.5) for all QUALIFIED profiles; those above `min_ready_to_connect_prob` (default 0.99) are promoted to READY_TO_CONNECT. During cold start (GP not fitted), no profiles reach READY_TO_CONNECT — the connect lane keeps triggering qualifications until the model can fit.
...
Cold start (< 2 labels or single class) returns `None` from `predict`/`bald_scores`/`predict_probs`, and `promote_to_ready()` returns 0. The connect task keeps triggering `qualify_one()` to accumulate labels until the GP model can fit.
...
- **`tasks/`** — Task handler functions executed by the daemon's task queue worker. All share the signature `handle_*(task, session, qualifiers, partner_qualifier, kit_model)`.
  - `connect.py` — `handle_connect`: regular campaigns only. Calls `get_candidate()` from `pools.py`, then connects. Self-reschedules with short delay (~10s). Enqueues `check_pending` on PENDING, `follow_up` on CONNECTED. On `ReachedConnectionLimit`, reschedules to next day. Also defines enqueue helpers: `enqueue_connect()`, `enqueue_check_pending()` (with multiplicative jitter), `enqueue_follow_up()` — all with dedup via existence check.
  - `connect_partner.py` — `handle_connect_partner`: partner campaigns only. Same connection flow as regular connect but uses `partner_qualifier` + `kit_model`, calls `seed_partner_deals()` each iteration. Self-reschedules with delay = `connect_delay_seconds / action_fraction` (scheduling-based rate control). Defines `enqueue_connect_partner()` with dedup.
...
- **`pipeline/`** — Candidate sourcing, qualification, and pool management:
  - `qualify.py` — Qualify orchestration. `qualify_one(session, qualifier)` calls `get_unlabeled_candidates(session)` for candidate sourcing, then selects via `qualifier.acquisition_scores()`, always queries LLM for decisions. Public: `get_unlabeled_candidates(session)` (returns unlabeled ProfileEmbedding rows, embedding one new lead if none exist). Private helpers: `_record_decision()` (updates `ProfileEmbedding` label, promotes or disqualifies lead), `_get_profile_text()` (enriches + builds text).
  ...
  - `ready_pool.py` — Ready-to-connect pool: GP confidence gate between QUALIFIED and READY_TO_CONNECT. `promote_to_ready(session, qualifier, threshold)` loads all QUALIFIED profiles; when `threshold <= 0` promotes all unconditionally (used by partner campaigns), otherwise computes P(f > 0.5) via GP model and promotes those above threshold; returns 0 on cold start. `get_ready_candidate(session, qualifier, pipeline=None)` returns the top-ranked READY_TO_CONNECT profile or None.
  - `pools.py` — Pool management via composable generators. Three generators chain via `next(upstream, None)`: `search_source(session)` yields keywords from `search_one()`, `qualify_source(session, qualifier)` yields public_ids from `qualify_one()` (in exploit mode, keeps searching via `_positive_pool_empty()` until candidates with P(f > 0.5) above `min_positive_pool_prob` exist — search auto-heals so never exhausts), `ready_source(session, qualifier, pipeline, threshold)` yields ready-to-connect candidates by checking `get_ready_candidate()` → `promote_to_ready()` → pulling from `qualify_source`. Each `qualify_source` iteration produces exactly one label, preventing infinite-search-without-qualifying. `_positive_pool_empty(qualifier, candidates)` uses adaptive threshold `max(0, base - 1/√n_obs)` — stays at zero until ~1/base² observations, then rises toward `min_positive_pool_prob`, favoring qualification over search early on; returns False on cold start, explore mode, empty candidates, or degenerate GP predictions (all identical P, detected via `np.ptp < 1e-6`). `get_candidate(session, qualifier, pipeline=None)` — derives `threshold` from `session.campaign.is_partner` (0.0 for partner, config default for regular) and calls `next(ready_source(...), None)`. Both partner and regular campaigns flow through the same `ready_source` chain; partner campaigns use `threshold=0` to bypass the GP confidence gate.
...
- **`ml/qualifier.py:BayesianQualifier`** — Pipeline(StandardScaler, GaussianProcessRegressor) with lazy refit. Operates on full 384-dim embeddings (no PCA). Training data is balanced before fitting: majority class is subsampled to at most `_MAX_IMBALANCE_RATIO` (2) times the minority class, preventing degenerate GP predictions on imbalanced labels. `update(embedding, label)` appends to training data and invalidates fit. `predict(embedding)` returns `(prob, entropy, posterior_std)` 3-tuple or `None` if unfitted. `predicted_probs(embeddings)` returns probability array. `compute_bald(embeddings)` computes BALD via MC sampling from GP posterior. `acquisition_scores(embeddings)` returns `(strategy_name, scores)` using balance-driven strategy (exploit: predict_probs when n_neg > n_pos, explore: BALD otherwise) or `None` on cold start. `pool_has_targets(embeddings)` returns `bool | None` — checks if pool has candidates useful to current strategy (exploit needs P > 0.5, explore needs P ≤ 0.5; None on cold start). `rank_profiles(profiles, session, pipeline=None)` sorts by P(f > 0.5) probability (descending); uses `load_embedding()` from `crm_profiles` for lazy embedding loading. `explain(profile, session)` returns human-readable explanation. `warm_start(X, y)` bulk-loads historical labels and fits once. Fitted pipeline persisted via `joblib` to per-campaign path (`model_path_for_campaign()`). Also exports `qualify_with_llm(profile_text, product_docs, campaign_objective)` for LLM-based lead qualification with structured output (`QualificationDecision`).
...
- **`sessions/account.py:AccountSession`** — Central session object. Loads `LinkedInProfile` from DB, exposes `linkedin_profile`, `campaign` (singular, set by daemon before each task), `campaigns` (property, all campaigns via group membership), `django_user`, `account_cfg` dict (handle, username, password, subscribe_newsletter, active, cookie_file), and Playwright browser (`page`, `context`, `browser`, `playwright`). Key methods: `ensure_browser()` (launches/recovers browser + login), `wait()` (human delay + page load), `_maybe_refresh_cookies()` (re-login if `li_at` cookie expired), `close()` (graceful teardown). Passed throughout the codebase.
...
- **`db/crm_profiles.py`** — Profile CRUD backed by DjangoCRM models. Lead-level functions: `lead_exists()`, `create_enriched_lead()`, `disqualify_lead()`, `promote_lead_to_contact()`, `get_leads_for_qualification()` (includes url-only leads), `count_leads_for_qualification()` (includes url-only leads). Lazy helpers: `ensure_lead_enriched(session, lead_id, public_id)` (Voyager API enrichment for url-only leads), `ensure_profile_embedded(lead_id, public_id, session)` (enrichment + fastembed as single lazy operation; enriches url-only leads first), `load_embedding(lead_id, public_id, session)` (loads embedding array, lazily enriching+embedding if needed), `lead_profile_by_id(lead_id)` (parse lead description by PK). Deal-level functions: `set_profile_state()`, `get_qualified_profiles()`, `count_qualified_profiles()`, `get_ready_to_connect_profiles()` (both `get_qualified_profiles` and `get_ready_to_connect_profiles` skip `lead__disqualified=False` filter for partner campaigns), `get_pending_profiles()`, `get_connected_profiles()`, `get_profile_dict_for_public_id(session, public_id)` (single profile lookup by public_id), `save_chat_message()`. URL helpers: `url_to_public_id(url)` (strict extractor, path must start with `/in/`), `public_id_to_url(public_id)`. Partner: `seed_partner_deals()`. Public helpers: `parse_next_step(deal)` (parse `deal.next_step` JSON, returns dict). Private helpers: `_make_ticket()` (uuid4 hex[:16]), `_fetch_profile()` (Voyager API wrapper), `_update_lead_fields()`, `_ensure_company()`, `_attach_raw_data()`. `set_profile_state()` clears `next_step` on actual transitions into/out of PENDING (not same-state). `count_qualified_profiles()` filters `lead__disqualified=False` for non-partner campaigns (consistent with `get_qualified_profiles()`).
...
- **`admin.py`** — Django Admin registrations: `CampaignAdmin` (list_display: department, booking_link, is_partner, action_fraction), `LinkedInProfileAdmin` (list_display: user, username, active; list_filter: active), `SearchKeywordAdmin` (list_display: keyword, campaign, used, used_at; list_filter: used, campaign), `ActionLogAdmin` (list_display: action_type, linkedin_profile, campaign, created_at; date_hierarchy: created_at; readonly), `TaskAdmin` (list_display: task_type, status, scheduled_at, payload, created_at; list_filter: task_type, status; date_hierarchy: scheduled_at; readonly). All use `raw_id_fields` for FK/O2O fields.
...
- **`conf.py:CAMPAIGN_CONFIG`** — Hardcoded timing/ML defaults:
  ...
  - `min_positive_pool_prob` (0.25) — P(f > 0.5) threshold for positive pool check in exploit mode
