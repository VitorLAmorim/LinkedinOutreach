...
4. **Qualify** (gap-filling) — two-phase with lazy self-healing: (1) embeds leads that lack embeddings — url-only leads are lazily enriched via Voyager API (`ensure_lead_enriched()`) before embedding (`ensure_profile_embedded()`), (2) qualifies embedded profiles using GPR active learning with balance-driven explore/exploit — when negatives outnumber positives, exploits (picks highest predicted probability); otherwise, explores (picks highest BALD score). LLM qualification also attempts lazy enrichment if profile text is missing. Predictive entropy + posterior std gate auto-decisions vs LLM queries → qualified or disqualified. Model lazily re-fitted on all accumulated labels when predictions are needed.
...
- **`lanes/`** — Action lanes executed by the daemon:
  - `qualify.py` — Two-phase qualification lane with lazy self-healing: (1) embeds leads that lack embeddings — url-only leads are lazily enriched via `ensure_lead_enriched()` before embedding via `ensure_profile_embedded()`, (2) qualifies embedded profiles via GPR active learning — balance-driven explore/exploit (more negatives → exploit highest prob, otherwise → explore highest BALD). `_get_profile_text(lead_id, public_id)` also attempts lazy enrichment if description is missing. Auto-decision requires both `entropy < threshold` AND `std < max_auto_std`; auto-accept also requires `prob >= min_accept_prob`. Otherwise → LLM query via `qualify_lead.j2` prompt. Reads `product_docs`/`campaign_objective` from `session.campaign`.
  - `connect.py` — Ranks qualified profiles by GPR predictive probability (via `BayesianQualifier.rank_profiles()`), sends connection requests. Rate-limits via `session.linkedin_profile.can_execute()`/`record_action()`/`mark_exhausted()`. Accepts optional `pipeline` kwarg for partner campaigns. Pre-existing connections always flow through as CONNECTED.
