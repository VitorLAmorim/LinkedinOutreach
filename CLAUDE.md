...
1. **Connect** (scheduled, generator chain) — Pull-based architecture. `execute()` calls `get_candidate()` from `pools.py` which uses composable generators: `ready_source` → `qualify_source` → `search_source`. Each level tries its own pool, then pulls ONE item from upstream if empty. `ready_source` checks READY_TO_CONNECT pool, then tries `promote_to_ready()`, then pulls from `qualify_source`. `qualify_source` in exploit mode keeps searching until P > 0.5 candidates exist or search is exhausted (avoids wasting LLM calls on low-P profiles), then qualifies exactly one profile per iteration. If a candidate is found, connects → PENDING. Partner campaigns bypass READY_TO_CONNECT and pick directly from QUALIFIED pool.
...
The qualification lane uses a **Gaussian Process Regressor** (sklearn, ConstantKernel * RBF) inside a `Pipeline(StandardScaler, GPR)` with BALD active learning:
...
- **`pipeline/`** — Candidate sourcing, qualification, and pool management:
  ...
  - `pools.py` — Pool management via composable generators. Three generators chain via `next(upstream, None)`: `search_source(session)` yields keywords from `search_one()`, `qualify_source(session, qualifier)` yields public_ids from `qualify_one()` (in exploit mode, keeps searching via `_positive_pool_empty()` until candidates with P(f > 0.5) above `min_positive_pool_prob` exist — search auto-heals so never exhausts), `ready_source(session, qualifier, pipeline)` yields ready-to-connect candidates by checking `get_ready_candidate()` → `promote_to_ready()` → pulling from `qualify_source`. Each `qualify_source` iteration produces exactly one label, preventing infinite-search-without-qualifying. `_positive_pool_empty(qualifier, candidates)` uses adaptive threshold `max(0, base - 1/√n_obs)` — stays at zero until ~1/base² observations, then rises toward `min_positive_pool_prob`, favoring qualification over search early on; returns False on cold start, explore mode, empty candidates, or degenerate GP predictions (all identical P, detected via `np.ptp < 1e-6`). `get_candidate(session, qualifier, pipeline=None, is_partner=False)` — partner path: queries `get_qualified_profiles()` directly, ranks, returns top (bypasses READY_TO_CONNECT). Regular path: `next(ready_source(...), None)`.
